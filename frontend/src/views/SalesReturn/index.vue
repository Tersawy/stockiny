	...
	<main-content :breads="breads">
		...
		<div style="overflow-x: auto" :class="`text-nowrap ${printsControl.table ? '' : 'd-print-none'}`">
			...
			>
				...
				<template #cell(reference)="row">
					<router-link :to="{ name: 'SaleReturn', params: { id: row.item._id } }" class="d-print-none">
						{{ row.value }}
					</router-link>
		...
		<div class="d-none d-print-block">
			...
			<template v-if="printsControl.payment">
				<SaleReturnPayment :paymentId="paymentId" />
			</template>
		</div>
...
const SaleReturnPayment = () => import("@/components/prints/SaleReturnPayment");
...
export default {
	...
	components: { SaleReturn, SaleReturnPayment, PrintHeader, InvoiceStatus, DateStr, InvoiceButtonFilter, DefaultModal, ChangeIcon, CheckCircleOutlineIcon },
	...
	computed: {
		...mapState({
			...
			statuses: (s) => s.SalesReturn.statuses,
			saleReturn: (s) => s.SalesReturn.one
		}),
	...
	methods: {
		getStatuses() {
			return this.$store.dispatch("SalesReturn/getStatuses");
		},
		...
		async changeStatus(item, status) {
			...
			try {
				await this.$store.dispatch("SalesReturn/changeStatus", { invoiceId: item._id, statusId: status._id });
		...
		btnCreateClicked() {
			this.$router.push({ name: "SaleReturnCreate" });
		},
		...
		async downloadPDF(invoice) {
			await this.$store.dispatch("SalesReturn/getOne", invoice._id);
		...
		async printPayment(payment) {
			if (this.saleReturn) {
				let res = { payments: this.saleReturn.payments || [], invoiceId: this.saleReturn._id };

				await this.$store.dispatch("SalesReturn/getOne", this.saleReturn._id);

				this.$store.commit("SalesReturn/payments", res);
			}
		...
		async printPayments(saleReturn) {
			this.$store.commit("SalesReturn/setOne", saleReturn);

			let getOne = this.$store.dispatch("SalesReturn/getOne", this.saleReturn._id);

			let getPayments = this.$store.dispatch("SalesReturn/getPayments");
