<template>
	<main-content :breads="breads">
			...
			<b-row>
				<b-col class="h-auto" cols="12" lg="8">
					<b-card header="Sale Information" class="h-100">
						<b-row cols="1" cols-sm="2">
							<!--------------- Date Input --------------->
							<b-col cols="12" sm="6">
								<default-date-picker-input label="Date" field="date" :vuelidate="$v.invoice" namespace="SalesReturn" />
							</b-col>

							<!--------------- Warehouse Input --------------->
							<b-col cols="12" sm="6">
								<default-select label="Warehouse" field="warehouse" :options="warehouseOptions" :vuelidate="$v.invoice" namespace="SalesReturn" />
							</b-col>

							<!--------------- Customer Input --------------->
							<b-col>
								<default-select label="Customer" field="customer" :options="customerOptions" :vuelidate="$v.invoice" namespace="SalesReturn" />
							</b-col>

							<!-- -------------Shipping Cost Input------------- -->
							<b-col>
								<default-input label="Shipping Cost" field="shipping" append="$" :vuelidate="$v.invoice" namespace="SalesReturn" type="number" />
							</b-col>

							<!-- -------------Tax Input------------- -->
							<b-col>
								<default-input label="Tax" field="tax" append="%" :vuelidate="$v.invoice" namespace="SalesReturn" type="number" />
							</b-col>

							<!-- -------------Discount------------- -->
							<b-col>
								<discount-input :vuelidate="$v.invoice" namespace="SalesReturn" />
							</b-col>
						</b-row>
					</b-card>
				</b-col>
				...
				<b-col cols="12" class="mt-4" order-lg="2">
					...
					<b-card header="Sale Details">
						...
						<input-error :vuelidate-field="$v.invoice.details" field="details" namespace="SalesReturn" />
				...
				<b-col cols="12" lg="4" class="mt-4 mt-lg-0" order-lg="1">
					...
					<b-card header="Sale Total">
						<!-- -------------Status------------- -->
						<invoice-status-input :vuelidate="$v.invoice" namespace="SalesReturn" @mounted="setEffectedStatus" />
				...
				<b-col class="mt-4" order="3">
					...
					<b-card header="Sale Notes">
						<default-text-area rows="4" class="mb-0" placeholder="Enter Sale Notes" field="notes" :vuelidate="$v.invoice" namespace="SalesReturn" />
					</b-card>
				</b-col>
			</b-row>
		</b-form>
		...
		<InvoiceDetailForm unitLabel="Sale Unit" namespace="SalesReturn" amountType="Price" :detail.sync="detail" @submit="updateDetail" />
	</main-content>
</template>
...
let detailsMixin = invoiceDetailsMixin({ storeNameSpace: "SalesReturn", checkQuantity: false, amountType: "Price" });

export default {
	mixins: [invoiceMixin("SalesReturn", "Price"), detailsMixin],

	data() {
		let isEdit = this.$route.params.id;

		return {
			breads: [{ title: "Dashboard", link: "/" }, { title: "Sales Return", link: "/sales-return" }, { title: isEdit ? "Edit" : "Create" }]
		};
	},

	computed: {
		productOptions() {
			return this.$store.getters["Products/options"]("saleReturn", this.invoice.warehouse);
		}
	},

	methods: {
		getProductOptions(payload) {
			return this.$store.dispatch("Products/getOptions", { ...payload, type: "saleReturn" });
		}
	}
};
</script>
