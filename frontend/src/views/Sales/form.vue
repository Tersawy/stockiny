<template>
	<main-content class="sale-form" :breads="breads">
			...
			<b-row>
				<b-col cols="9">
					...
					<b-card class="my-30px">
						<products-auto-complete :invoice="invoice" :amountType="amountType" check-quantity />
						<input-error :vuelidate="this.$v.invoice.products" field="products" :namespace="namespace" />
					</b-card>

					<!-- -------------Products Table------------- -->
					<b-card>
						<invoice-details-table :invoice="invoice" :amountType="amountType" :namespace="namespace" :net="net" check-quantity />
					</b-card>
...
<script>
	...
	export default {
		...
		data() {
			return { namespace: "Sales", amountType: "price" };
		},
		...
		computed: {
			breads() {
				...
				if (this.isUpdate) {
					breads = [...breads, { title: this.oldInvoice.reference, link: `/sales/${this.invoiceId}` }, { title: "Edit" }];
				} else {
