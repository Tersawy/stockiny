<!-- eslint-disable -->
<template>
	<svg
		version="1.1"
		:width="width"
		:height="height"
		v-on="$listeners"
		xmlns="http://www.w3.org/2000/svg"
		xmlns:xlink="http://www.w3.org/1999/xlink"
		x="0px"
		y="0px"
		viewBox="0 0 172.358 172.359"
		style="enable-background: new 0 0 172.358 172.359"
		xml:space="preserve"
	>
		<g>
			<g>
				<path
					d="M58.28,83.67V30.426c0-1.393-1.138-2.524-2.524-2.524h-4.25c-1.386,0-2.524,1.132-2.524,2.524v4.406
			c0,1.392-1.132,2.523-2.518,2.523H30.402v-6.312h-2.524v6.312H11.813c-1.386,0-2.518-1.132-2.518-2.523v-4.406
			c0-1.393-1.138-2.524-2.524-2.524H2.524C1.138,27.902,0,29.034,0,30.426V83.67c0,1.38,1.138,2.506,2.524,2.506h53.238
			C57.143,86.176,58.28,85.044,58.28,83.67z"
				/>
				<path
					d="M27.873,28.538c15.276,4.406,8.78-19.695,3.629-21.421c0,0,0,8.964-3.629,5.973c-3.263-2.686-1.103-7.716,1.259-11.487
			C22.358,4.903,12.596,24.119,27.873,28.538z"
				/>
				<path
					d="M169.828,27.902h-4.244c-1.389,0-2.529,1.132-2.529,2.524v4.406c0,1.392-1.129,2.523-2.518,2.523h-16.066v-6.312h-2.518
			v6.312h-16.065c-1.383,0-2.518-1.132-2.518-2.523v-4.406c0-1.393-1.136-2.524-2.524-2.524h-4.25c-1.383,0-2.523,1.132-2.523,2.524
			V83.67c0,1.38,1.141,2.506,2.523,2.506h53.244c1.384,0,2.519-1.132,2.519-2.506V30.426
			C172.353,29.034,171.224,27.902,169.828,27.902z"
				/>
				<path
					d="M141.953,28.538c15.273,4.406,8.783-19.695,3.629-21.421c0,0,0,8.964-3.629,5.973c-3.263-2.686-1.1-7.716,1.265-11.487
			C136.438,4.903,126.668,24.119,141.953,28.538z"
				/>
				<path
					d="M112.783,112.482h-4.25c-1.383,0-2.523,1.129-2.523,2.524v4.409c0,1.389-1.129,2.518-2.512,2.518H87.429v-6.312h-2.524
			v6.312H68.843c-1.386,0-2.518-1.129-2.518-2.518v-4.409c0-1.396-1.138-2.524-2.524-2.524h-4.25c-1.386,0-2.524,1.129-2.524,2.524
			v53.244c0,1.383,1.138,2.506,2.524,2.506h53.238c1.384,0,2.524-1.135,2.524-2.506v-53.244
			C115.308,113.611,114.173,112.482,112.783,112.482z"
				/>
				<path
					d="M84.905,113.115c15.276,4.397,8.783-19.701,3.632-21.421c0,0,0,8.961-3.632,5.97c-3.263-2.684-1.096-7.719,1.265-11.487
			C79.396,89.484,69.629,108.699,84.905,113.115z"
				/>
			</g>
		</g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
		<g></g>
	</svg>
</template>
<!-- eslint-disable -->

<script>
	export default {
		props: {
			width: {
				type: String,
				default: "0.9rem"
			},
			height: {
				type: String,
				default: "0.9rem"
			},
			color: {
				type: String,
				default: "#333"
			}
		}
	};
</script>
