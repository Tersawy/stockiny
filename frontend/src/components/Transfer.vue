<template>
	<b-row v-if="transfer && transfer._id">
		<b-col>
			<b-card>
				<b-row>
				...
				<b-row cols="1" cols-md="2" cols-lg="3" class="order-info">
					<b-col>
						<h6 class="fs-14 text-muted mb-1">From Warehouse</h6>
						<h6 class="fs-14 mb-1">{{ transfer.fromWarehouse.name | toSentenceCase }}</h6>
						<div class="fs-14">{{ transfer.fromWarehouse.address }}</div>
						<div class="fs-14">{{ transfer.fromWarehouse.city }} - {{ transfer.fromWarehouse.country }}</div>
						<div class="fs-14">{{ transfer.fromWarehouse.zipCode }}</div>
						<div class="fs-14">{{ transfer.fromWarehouse.phone }}</div>
						<div class="fs-14">{{ transfer.fromWarehouse.email }}</div>
					</b-col>
					<hr style="width: 96%" class="d-md-none d-print-none" />
					<b-col>
						<h6 class="fs-14 text-muted mb-1">To Warehouse</h6>
						<h6 class="fs-14 mb-1">{{ transfer.toWarehouse.name | toSentenceCase }}</h6>
						<div class="fs-14">{{ transfer.toWarehouse.address }}</div>
						<div class="fs-14">{{ transfer.toWarehouse.city }} - {{ transfer.toWarehouse.country }}</div>
						<div class="fs-14">{{ transfer.toWarehouse.zipCode }}</div>
						<div class="fs-14">{{ transfer.toWarehouse.phone }}</div>
						<div class="fs-14">{{ transfer.toWarehouse.email }}</div>
					</b-col>
					...
					<b-col cols="12" md="12">
						<b-row cols="2" class="text-nowrap">
							...
							<b-col class="mb-1">
								<DateStr v-if="transfer.date" :date="transfer.date" />
							</b-col>
							<b-col class="mb-1 text-lg-right">
								<h6 class="fs-14 mb-0">Status:</h6>
							</b-col>
							<b-col class="mb-1">
								<span v-if="!transfer.status || typeof transfer.status == 'string'"> - - -</span>
								<InvoiceStatus v-else :status="transfer.status" />
							</b-col>
							<b-col class="mb-1 text-lg-right">
								<h6 class="fs-14 mb-0">Requested at:</h6>
							</b-col>
							<b-col class="mb-1">
								<DateStr v-if="transfer.createdAt" :date="transfer.createdAt" />
							</b-col>
							<b-col class="mb-1 text-lg-right">
								<h6 class="fs-14 mb-0">Requested by:</h6>
							</b-col>
							<b-col class="mb-1">
								<span v-if="transfer.createdBy">{{ transfer.createdBy.fullname }}</span>
							</b-col>
						</b-row>
					</b-col>
				</b-row>
							...
							<tbody>
								<tr>
									...
									<td class="border-0">
										...
										<span class="text-nowrap"> ( {{ transfer.tax | floating }}% ) </span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>$ {{ transfer.shipping | floating }}</td>
								</tr>
								<tr>
...
export default {
	...
	computed: {
		...
		details() {
			if (!this.transfer.details) return [];

			return this.transfer.details.map((item) => {
				return {
					...item,
		...
		discountFixed() {
			let discount = +this.transfer.discount || 0;

			if (!discount) return 0;

			let isFixed = this.transfer.discountMethod == "fixed";
		...
		discountPercent() {
			...
			let isPercent = this.transfer.discountMethod == "percent";
		...
		orderTotal() {
			return this.totalPriceWithoutDiscount + this.taxFixed + +this.transfer.shipping;
		}
	},
