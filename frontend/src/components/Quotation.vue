<template>
	<b-row v-if="quotation && quotation._id">
		<b-col>
			<b-card>
				<b-row>
				...
				<b-row class="order-total justify-content-end mt-3">
					<b-col v-if="withPayments && quotation.payments && quotation.payments.length">
						<b-table :fields="paymentsFields" show-empty :items="quotation.payments" class="mb-0 text-center" bordered>
							...
							<tbody>
								<tr>
									...
									<td class="border-0">
										...
										<span class="text-nowrap"> ( {{ quotation.tax | floating }}% ) </span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>$ {{ quotation.shipping | floating }}</td>
								</tr>
								<tr>
...
export default {
	props: {
		...
		withPayments: { type: Boolean, default: false }
	},
	...
	data() {
		return {
			...
			paymentsFields: [{ key: "date", label: "Date", formatter: getDate }, "reference", "amount", { key: "paymentType", label: "Paid By" }]
		};
	},

	computed: {
		quotation() {
			if (this.invoice && this.invoice._id) return this.invoice;

			return this.$store.state.Quotations.one;
		},

		details() {
			if (!this.quotation.details) return [];

			return this.quotation.details.map((item) => {
				return {
					...item,
		...
		discountFixed() {
			let discount = +this.quotation.discount || 0;

			if (!discount) return 0;

			let isFixed = this.quotation.discountMethod == "fixed";
		...
		discountPercent() {
			...
			let isPercent = this.quotation.discountMethod == "percent";
		...
		orderTotal() {
			return this.totalPriceWithoutDiscount + this.taxFixed + +this.quotation.shipping;
		},

		due() {
			return this.orderTotal - +this.quotation.paid;
		}
	},
