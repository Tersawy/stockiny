<template>
	<b-row v-if="purchaseReturn && purchaseReturn._id">
		<b-col>
			<b-card>
				<b-row>
							...
							<tbody>
								<tr>
									...
									<td class="border-0">
										...
										<span class="text-nowrap"> ( {{ purchaseReturn.tax | floating }}% ) </span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>$ {{ purchaseReturn.shipping | floating }}</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>$ {{ purchaseReturn.paid | floating }}</td>
								</tr>
								<tr>
...
export default {
	...
	computed: {
		...
		details() {
			if (!this.purchaseReturn.details) return [];

			return this.purchaseReturn.details.map((item) => {
				return {
					...item,
		...
		discountFixed() {
			let discount = +this.purchaseReturn.discount || 0;

			if (!discount) return 0;

			let isFixed = this.purchaseReturn.discountMethod == "fixed";
		...
		discountPercent() {
			...
			let isPercent = this.purchaseReturn.discountMethod == "percent";
		...
		taxFixed() {
			let tax = +this.purchaseReturn.tax || 0;
		...
		orderTotal() {
			return this.totalPriceWithoutDiscount + this.taxFixed + +this.purchaseReturn.shipping;
		},

		due() {
			return this.orderTotal - +this.purchaseReturn.paid;
		}
	},
